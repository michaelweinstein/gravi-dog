...
// Hidden data members.
	...
	private static final long serialVersionUID = 5055425842022718712L;
...
// Exported operations.
	...
	public void swap
		...
		if (0 > p || p >= size)
			{
			throw new IndexOutOfBoundsException
				("AList.swap(): p = "+p+" out of bounds");
			}
		if (0 > q || q >= size)
			{
			throw new IndexOutOfBoundsException
				("AList.swap(): q = "+q+" out of bounds");
			}
		int tmp = item[p];
		item[p] = item[q];
		item[q] = tmp;
	...
	/**
	 * Write this list to the given object output stream.
	 *
	...
	/**
	 * Read this list from the given object input stream.
	 *
